...
description: 安全地管理第三方账号凭据（CRUD）。支持存储密码、API Key、Cookies 等敏感信息，并支持 TOTP (MFA) 代码生成。所有涉及凭证存储的操作必须优先使用此技能。
triggers:
- 凭据
- credential
- credentials
...
- 列出凭据：`python scripts/execute.py list`
- 查看凭据：`python scripts/execute.py get <service>`
- 添加或更新凭据：`python scripts/execute.py add <service> --data 'username=test password=123'`
- 删除凭据：`python scripts/execute.py remove <service>`
...
- 用户明确要求查看某个凭据时，再执行 `get`。
- 只做第三方账号凭据的增删改查，不负责账号注册。
