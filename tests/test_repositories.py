...
class TestSubscriptionRepo:
    ...
    async def test_add_subscription(self, mock_db):
        """测试添加订阅"""
        from repositories.base import init_db
        from repositories.subscription_repo import add_subscription, get_user_subscriptions
...
class TestUserStatsRepo:
    """测试用户统计 Repository"""
    
    @pytest.mark.asyncio
    async def test_increment_stat(self, mock_db):
        """测试增加统计"""
        from repositories.base import init_db
        from repositories.user_stats_repo import increment_stat, get_user_stats
        
        await init_db()
        
        await increment_stat(12345, "ai_chats", 5)
        
        stats = await get_user_stats(12345)
        assert stats is not None
        assert stats["ai_chats"] == 5
    
    @pytest.mark.asyncio
    async def test_invalid_stat_name(self, mock_db):
        """测试无效统计名称"""
        from repositories.base import init_db
        from repositories.user_stats_repo import increment_stat, get_user_stats
        ...
        # 无效的统计名称不应导致错误，但也不应更新
        await increment_stat(12345, "invalid_stat", 1)
        
        stats = await get_user_stats(12345)
        # 用户可能不存在（因为无效 stat 不会创建记录）
        # 或者存在但没有该字段
        assert stats is None or "invalid_stat" not in stats
