...
## OVERVIEW
`tests/core/` validates manager orchestration loops, tool dispatch contracts, worker queue/runtime behavior, and core reliability boundaries.
...
| Extension and dispatch contracts | `tests/core/test_extension_router.py`, `tests/core/test_extension_executor.py`, `tests/core/test_dispatch_tools.py`, `tests/core/test_skill_arg_planner.py`, `tests/core/test_skill_loader_schema_inference.py` | tool-call and skill execution compatibility |
| Handler-level routing guarantees | `tests/core/test_ai_handlers_dispatch.py`, `tests/core/test_worker_handlers_filters.py`, `tests/core/test_heartbeat_handlers.py`, `tests/core/test_start_handlers_stop.py`, `tests/core/test_unified_context_reply.py` | command/message routing and control-path behavior |
| Worker queue and relay behavior | `tests/core/test_worker_runtime.py`, `tests/core/test_worker_store.py`, `tests/core/test_worker_task_file_store.py`, `tests/core/test_worker_result_relay.py` | worker lifecycle, queue contracts, async delivery |
| Prompt, memory, and state stores | `tests/core/test_prompt_composer.py`, `tests/core/test_markdown_memory_store.py`, `tests/core/test_soul_store.py`, `tests/core/test_waiting_phrase_store.py` | prompt chain integrity and persistence behavior |
...
uv run pytest tests/core/test_orchestrator_single_loop.py tests/core/test_worker_result_relay.py
```
