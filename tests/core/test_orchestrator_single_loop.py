...
async def test_orchestrator_default_tools_are_primitives(monkeypatch):
    ...
    async def fake_stream(
        ...
        captured["tool_names"] = [
            tool["name"] if isinstance(tool, dict) else tool.name for tool in (tools or [])
        ]
        yield "ok"
    ...
    assert chunks == ["ok"]
    assert captured["tool_names"] == ["read", "write", "edit", "bash"]
    assert "call_skill" not in captured["tool_names"]


@pytest.mark.asyncio
async def test_orchestrator_injects_short_lived_extension(monkeypatch):
    orchestrator = AgentOrchestrator()
    captured = {}
    ...
    monkeypatch.setattr(orchestrator.ai_service, "generate_response_stream", fake_stream)
    monkeypatch.setattr(orchestrator.extension_router, "route", lambda *_args, **_kwargs: [candidate])
    monkeypatch.setattr(orchestrator_module, "MCP_MEMORY_ENABLED", False)
    ...
    assert "ext_rss_subscribe" in captured["tool_names"]


@pytest.mark.asyncio
async def test_orchestrator_injects_deployment_without_skill_manager_by_default(monkeypatch):
    orchestrator = AgentOrchestrator()
    ...
    assert "ext_deployment_manager" in captured["tool_names"]
    assert "ext_skill_manager" not in captured["tool_names"]


@pytest.mark.asyncio
async def test_orchestrator_keeps_deployment_extension_when_explicitly_requested(monkeypatch):
    orchestrator = AgentOrchestrator()
    orchestrator.direct_fastpath_enabled = False
    captured = {}
    ...
    ctx = DummyContext()
    message_history = [{"role": "user", "parts": [{"text": "请使用 deployment_manager 部署一套n8n"}]}]
...
async def test_orchestrator_injects_skill_manager_for_skill_intent(monkeypatch):
    ...
    ctx = DummyContext()
    message_history = [{"role": "user", "parts": [{"text": "帮我创建一个新技能处理这个任务"}]}]

    _ = [chunk async for chunk in orchestrator.handle_message(ctx, message_history)]

    assert "ext_skill_manager" in captured["tool_names"]
...
async def test_orchestrator_prompt_includes_runtime_and_skill_briefs(monkeypatch):
    ...
    assert "ext_deployment_manager" in captured["system_instruction"]
...
@pytest.mark.asyncio
async def test_orchestrator_direct_exec_for_explicit_deep_research(monkeypatch):
    orchestrator = AgentOrchestrator()
    ...
    candidate = ExtensionCandidate(
        ...
        schema_summary="required=['topic'], fields=['topic', 'depth', 'language']",
    ...
    ctx = DummyContext()
    message_history = [{"role": "user", "parts": [{"text": "帮我深度研究 minimax 2.5 最新发布"}]}]

    chunks = [chunk async for chunk in orchestrator.handle_message(ctx, message_history)]

    assert chunks == ["执行完成"]
    assert stream_called["value"] is False
    assert captured["skill_name"] == "deep_research"
    assert captured["args"]["topic"]
    assert captured["args"]["depth"] == 5


@pytest.mark.asyncio
async def test_orchestrator_direct_exec_for_explicit_deployment(monkeypatch):
    orchestrator = AgentOrchestrator()
    orchestrator.direct_fastpath_enabled = True
    captured = {}
    stream_called = {"value": False}
    ...
    candidate = ExtensionCandidate(
        ...
        input_schema={
            ...
            "properties": {
                ...
                "request": {"type": "string"},
                "service": {"type": "string"},
                "host_port": {"type": "integer", "default": 23001},
            },
            "required": ["action"],
        },
        schema_summary="required=['action'], fields=['action', 'request', 'service', 'host_port']",
        triggers=["部署", "deploy"],
    )
    ...
    ctx = DummyContext()
    message_history = [{"role": "user", "parts": [{"text": "帮我部署 uptime kuma 端口 23011"}]}]

    chunks = [chunk async for chunk in orchestrator.handle_message(ctx, message_history)]

    assert chunks == ["✅ 部署完成"]
    assert stream_called["value"] is False
    assert captured["skill_name"] == "deployment_manager"
    assert captured["args"]["action"] == "auto_deploy"
    assert captured["args"]["service"] == "uptime-kuma"
    assert captured["args"]["host_port"] == 23011


@pytest.mark.asyncio
async def test_orchestrator_direct_exec_for_n8n_deployment(monkeypatch):
    orchestrator = AgentOrchestrator()
    orchestrator.direct_fastpath_enabled = True
    captured = {}
    ...
    async def fake_execute(skill_name, args, ctx, runtime):
        captured["skill_name"] = skill_name
        captured["args"] = args
        return ExtensionRunResult(ok=True, skill_name=skill_name, text="ok")

    async def fake_stream(*_args, **_kwargs):
        raise AssertionError("stream should not be called for direct deployment path")
    ...
    assert captured["skill_name"] == "deployment_manager"
    assert captured["args"]["action"] == "auto_deploy"
    assert captured["args"]["service"] == "n8n"
...
@pytest.mark.asyncio
async def test_orchestrator_direct_exec_on_followup_context(monkeypatch):
    orchestrator = AgentOrchestrator()
    orchestrator.direct_fastpath_enabled = True
    captured = {}
    ...
    async def fake_stream(*_args, **_kwargs):
        raise AssertionError("stream should not run on follow-up direct execution")
    ...
    message_history = [
        {"role": "user", "parts": [{"text": "帮我做深度研究 MiniMax 2.5"}]},
        {"role": "model", "parts": [{"text": "请补充你关心的内容"}]},
        {"role": "user", "parts": [{"text": "我指的是 minimax 2.5 最新发布"}]},
    ]
    ...
    assert chunks == ["ok"]
    assert captured["skill_name"] == "deep_research"
    assert "最新发布" in captured["args"]["topic"]
...
@pytest.mark.asyncio
async def test_orchestrator_skips_auto_evolve_when_non_skill_extension_exists(monkeypatch):
    orchestrator = AgentOrchestrator()
    orchestrator.direct_fastpath_enabled = False
    orchestrator.auto_evolve_enabled = True
    call_state = {"count": 0}
