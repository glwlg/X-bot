...
import re
from telegram import Update as LegacyUpdate # Keep for legacy compatibility if needed, but we try to avoid it
...
class DiscordAdapter(BotAdapter):
    ...
    def __init__(self, token: str):
        ...
        self._callback_handlers: List[Tuple[re.Pattern, Callable[[UnifiedContext], Any]]] = []
    ...
    def _setup_events(self):
        @self.client.event
        async def on_ready():
            logger.info(f'Logged in as Discord Bot: {self.client.user} (ID: {self.client.user.id})')
    ...
    async def _map_message(self, message: discord.Message) -> UnifiedMessage:
        ...
        if message.attachments:
            ...
            ct = att.content_type
            if ct:
                if ct.startswith('image'): msg_type = MessageType.IMAGE
                elif ct.startswith('video'): msg_type = MessageType.VIDEO
                elif ct.startswith('audio'): msg_type = MessageType.AUDIO
                else: msg_type = MessageType.DOCUMENT
        ...
        return UnifiedMessage(
            ...
            chat=Chat(
                ...
                type="group" if isinstance(message.channel, (discord.GroupChannel, discord.TextChannel)) else "private",
            ...
            reply_to_message=await self._map_message(message.reference.resolved) if message.reference and isinstance(message.reference.resolved, discord.Message) else None,
        )

    def _telegram_markup_to_discord_view(self, reply_markup: Any) -> Optional[discord.ui.View]:
        ...
        try:
             ...
             if interaction.message:
                 ...
                 unified_msg = UnifiedMessage(
                     id="interaction", platform="discord", type=MessageType.TEXT,
                     user=User(id=str(interaction.user.id), username=interaction.user.name),
                     chat=Chat(id=str(interaction.channel_id), type="private"), # minimal
    ...
    async def reply_text(self, context: UnifiedContext, text: str, **kwargs) -> Any:
        try:
            ...
            # TODO: Format Markdown to Discord Style if needed (Telegram MD vs Discord MD)
            # For now passing text raw
    ...
    async def reply_photo(self, context: UnifiedContext, photo: Any, caption: str = None, **kwargs) -> Any:
        try:
            channel = context.platform_event.channel
    ...
    async def reply_video(self, context: UnifiedContext, video: Any, caption: str = None, **kwargs) -> Any:
        try:
            channel = context.platform_event.channel
    ...
    async def reply_audio(self, context: UnifiedContext, audio: Any, caption: str = None, **kwargs) -> Any:
        try:
            channel = context.platform_event.channel
    ...
    async def reply_document(self, context: UnifiedContext, document: Any, filename: str = "document", caption: str = None, **kwargs) -> Any:
        try:
            channel = context.platform_event.channel
    ...
    async def _handle_slash_command(self, interaction: discord.Interaction, command: str, params: Optional[str], handler: Callable):
        """Generic wrapper for slash commands"""
        try:
             # Defer if needed? Or let handler do it? Handler usually calls ctx.reply which we adapted to use response.send_message.
             # But if handler takes too long (>3s) without reply, we crash.
             # Safer to defer immediately?
             # await interaction.response.defer(thinking=True) 
             # But if we defer, ctx.reply must use followup.
             # Let's defer only if we suspect long running?
             # For now, let's NOT defer automatically, to allow instant reply. Handler should reply fast or use send_chat_action.
             ...
             if interaction.channel:
                  if isinstance(interaction.channel, (discord.GroupChannel, discord.TextChannel)):
                      chat_type = "group"
    ...
    def on_command(self, command: str, handler: Callable[[UnifiedContext], Any]):
        ...
        async def slash_callback(interaction: discord.Interaction, args: Optional[str] = None):
    ...
    def get_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        logger.info(f"[DiscordUserData] Access for {user_id}. Current keys: {list(data.keys())}")
        return data
