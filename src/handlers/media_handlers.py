        ...
        try:
            if audio_only:
                ...
                if hasattr(sent_message, "video") and sent_message.video:
                    ...
                    # Discord might send as document if huge? Or generic attachment
...
async def handle_video_actions(ctx: UnifiedContext) -> None:
    ...
    logger.info(
        f"[VideoAction] User {ctx.message.user.id} checking user_data. Keys: {list(ctx.user_data.keys())}"
    )
    ...
    if action == "action_download_video":
        ...
        # 调用下载逻辑
        result = await download_video(
            url, chat_id, processing_message, audio_only=False
        )
        ...
        if result.is_too_large:
            ...
            keyboard = [
                [
                    ...
                    InlineKeyboardButton(
                        "🎵 仅发送音频", callback_data="large_file_audio"
                    ),
                ],
                [
                    InlineKeyboardButton(
                        "🗑️ 删除文件", callback_data="large_file_delete"
                    ),
                ],
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    try:
        if data == "large_file_delete":
            ...
            if os.path.getsize(final_path) > 50 * 1024 * 1024:
                await ctx.edit_message(
                    ctx.message.id, f"❌ 提取的音频也超过 50MB，无法发送。"
                )
            else:
