...
async def handle_subscription_callback(ctx: UnifiedContext):
    ...
    try:
        if data.startswith("del_rss_"):
            ...
            if success:
                await ctx.answer_callback("✅ 订阅已删除")
            else:
                await ctx.answer_callback("❌ 删除失败，可能已不存在", show_alert=True)
            ...
            if success:
                await ctx.answer_callback("✅ 自选股已删除")
            else:
        ...
        await ctx.answer_callback("❌ 系统错误", show_alert=True)


async def refresh_subscription_list_message(
    ctx: UnifiedContext, message_id: str, user_id: int
):
    """
    刷新订阅列表消息内容 (删除后更新 UI)
    """
    ...
    if rss_subs:
        ...
        for sub in rss_subs:
            ...
            short_title = (
                sub["title"][:8] + ".." if len(sub["title"]) > 8 else sub["title"]
            )
            btn = InlineKeyboardButton(
                f"❌ {short_title}", callback_data=f"del_rss_{sub['id']}"
            )
    ...
    if stocks:
        ...
        for s in stocks:
            ...
            short_name = (
                s["stock_name"][:8] + ".."
                if len(s["stock_name"]) > 8
                else s["stock_name"]
            )
            btn = InlineKeyboardButton(
                f"❌ {short_name}", callback_data=f"del_stock_{s['stock_code']}"
            )
