...
### 🤖 强大的 AI 能力
-   **多轮智能对话**：基于 Gemini 3.0 Flash 模型，支持上下文记忆（自动保存最近 10 条对话）。
-   **多模态分析**：
...
-   **智能路由**：直接发送视频/社交媒体链接（如 YouTube, X），Bot 会贴心询问是 **下载视频** 还是 **生成摘要**。
-   **深度解析**：内置 Google News 解码器与反爬虫兜底策略，确保能获取到最真实的网页内容。

### ⏰ 智能提醒与交互
-   **交互式命令**：忘记加参数？没关系！直接发送 `/remind`、`/monitor` 或 `/subscribe`，Bot 会引导你完成后续输入。
-   **自然语言设置**：`/remind 10m 喝水` 或 `/remind 2h 开会`。
-   **精准触达**：即使 Bot 重启，提醒任务也不会丢失。
