...
### � 容器与部署管理 (New)
-   **自然语言部署**：
    -   发送 "部署这个仓库 https://github.com/..."，Bot 自动拉取代码、检测配置、解决端口冲突并启动服务。
    -   支持 Dockerfile 和 Docker Compose 项目。
-   **服务管理**：
...
-   **所想即所得**：无需死记硬背 `/download` 或 `/remind`，直接说出你的需求。
-   **智能决策**：
...
-   **安全沙箱**：自动生成 Python 代码并在受限环境中运行。
-   **自动发现**：当你提出无法满足的需求时（如“计算MD5”），Agent 会尝试从市场搜索并安装新技能。
